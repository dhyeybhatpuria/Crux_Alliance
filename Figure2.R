...
output <- master_selected %>% group_by(Year, Recipient) %>% summarise(Commitment = sum(Mitigation))
output['Finance.Instrument'] = unique(master_selected$Finance.Instrument)


titletext = paste("Year-wise commitment by ", countryname ," (2015-2019)")
gg = ggplot(output, aes(x=Year, y=Commitment, group=Recipient, color=Recipient)) + #geom_line(data=dfr[!is.na(dfr$y),])
...
ggplot(output, aes(fill=Recipient, y=Commitment, x=Year)) + 
